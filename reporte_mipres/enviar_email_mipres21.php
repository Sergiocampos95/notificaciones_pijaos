...
################################################################################
################################################################################
#################      ARMADO PRESCRIPCIONES A�O 2021      #####################


        $consulta_direccionamiento = $reporte->get_direccionamientos('2021-01-01', '2021-12-31');
        ...
        $archivo_direccionamientos = ($data_direccionamientos) ? generar_arc_direccionamientos21($reporte->get_data_direccionamientos('2021-01-01', '2021-12-31')) : '0';

################################################################################
################################################################################
################################################################################
#############    ARMADO PRESCRIPCIONES A�O 2021 CON URG Y HOSP    ##############


        $consulta_direUH = $reporte->get_direUH('2021-01-01', '2021-12-31');
        $data_direUH = array();
        $sum_reg_dirUH = 0;
        ...
        $archivo_direccionamientosUH = ($data_direUH) ? generar_arc_direUH21($reporte->get_data_direUH('2021-01-01', '2021-12-31')) : '0';
        ...
        if (count($data_direccionamientos) > 0) {
            ...
            try {
                ...
                $tabla_msg = "<p style='text-align: justify'> "
                        . "Consolidado registros MIPRES de la vigencia 2021 con fecha m&aacute;xima de entrega menor a la actual y sin "
                        . "reporte de entrega por parte del proveedor de servicios."
                        . "</p>"
                        . "<table border='1' cellspacing='0' cellpadding='2' style='width: 50%;'> "
                        . "<thead> "
                        . "<tr> "
                        . "<th colspan='4' style='background-color: #e1edd7'>Prescripciones Vigencia 2021 - (Se excluyen servicios con &aacute;mbito hospitalizaci&oacute;n y urgencias) </th> "
                        . "</tr> "
                        . " <tr> "
                        ...
                        . "<thead> "
                        . "<tr> "
                        . "<th colspan='4' style='background-color: #e1edd7'>Prescripciones Vigencia 2021 - Trazabilidad solo Urgencias y Hospitalizaci&oacute;n</th> "
                        . "</tr> "
                        . " <tr> "
                        . "<th style='background-color: #e1edd7'>Tip_ident.</th> "
                        . "<th style='background-color: #e1edd7'>Num_ident.</th> "
                ...
                $mail->Subject = utf8_decode("Notificacion Diaria - Registros MIPRES sin reporte de entrega vigencia 2021");
                ...
                log_txt('log_mipres21.txt', "Mensaje enviado con exito!!!");
            } catch (Exception $e) {
                ...
                log_txt('log_mipres21.txt', "Error al enviar el mensaje: {$mail->ErrorInfo}");
            }
        } else {

            //echo 'No hay resultados para exportar';
            log_txt('log_mipres21.txt', "No hay resultados para exportar");
        }
    } else {

        log_txt('log_mipres21.txt', utf8_encode('Llave de verificacion invalida. Consulte con el administrador de la aplicacion'));
        echo utf8_encode('Llave de verificacion invalida. Consulte con el administrador de la aplicacion');
    }
} else {

    log_txt('log_mipres21.txt', utf8_encode('Recurso inaccesible, tarea programada en el servidor 192.168.20.240'));
